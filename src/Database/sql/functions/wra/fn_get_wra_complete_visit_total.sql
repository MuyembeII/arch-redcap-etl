/**
 * Returns WRAs total number of completed visits.
 *
 * @author Gift Jr <muyembegift@gmail.com> | 15.01.25
 * @since 0.0.1
 * @alias Get WRA Tx Completed Visits.
 * @param varchar WRA PTID
 * @return Tx Number of Completed Visits
 */
DROP FUNCTION IF EXISTS `get_WRA_Tx_Completed_Visits`;
DELIMITER $$
CREATE FUNCTION get_WRA_Tx_Completed_Visits(p_wra_ptid VARCHAR(6))
    RETURNS INT
    NOT DETERMINISTIC
BEGIN
    ...
    IF @v_first_follow_up_visit_outcome = 'Completed' THEN
        SET v_total = 1;
    ELSE
        SET v_total = v_total;
    END IF;

    IF @v_second_follow_up_visit_outcome = 'Completed' THEN
        SET v_total = v_total + 1;
    ELSE
        SET v_total = v_total;
    END IF;

    IF @v_third_follow_up_visit_outcome = 'Completed' THEN
        SET v_total = v_total + 1;
    ELSE
        SET v_total = v_total;
    END IF;

    IF @v_fourth_follow_up_visit_outcome = 'Completed' THEN
        SET v_total = v_total + 1;
    ELSE
        SET v_total = v_total;
    END IF;
