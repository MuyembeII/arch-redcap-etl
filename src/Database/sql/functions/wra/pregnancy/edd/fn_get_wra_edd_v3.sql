 ...
 * @return DATE | EDD
 */
DROP FUNCTION IF EXISTS `getEstimatedDateOfDelivery_V3`;
DELIMITER $$
CREATE FUNCTION arch_etl_db.getEstimatedDateOfDelivery_V3(p_record_id BIGINT)
    RETURNS DATE
    READS SQL DATA
    NOT DETERMINISTIC
BEGIN
    ...
    SELECT COALESCE(pos_v3.ps_fu_visit_date_f2, v3.visit_date),
           pos_v3.fu_lmp_scdat_f2,
           CASE pos_v3.fu_lmp_cat_scorres_f2
               WHEN 0 THEN pos_v3.fu_lmp_start_weeks_f2
               WHEN 1 THEN pos_v3.fu_lmp_start_months_f2
               WHEN 2 THEN pos_v3.fu_lmp_start_years_f2 END,
           CASE pos_v3.fu_lmp_cat_scorres_f2
               WHEN 0 THEN 'week(s)'
               WHEN 1 THEN 'month(s)'
    ...
    FROM wrafu_pregnancy_surveillance_2 pos_v3
             JOIN arch_etl_db.crt_wra_visit_3_overview v3 on pos_v3.record_id = v3.record_id
    WHERE pos_v3.record_id = p_record_id;
