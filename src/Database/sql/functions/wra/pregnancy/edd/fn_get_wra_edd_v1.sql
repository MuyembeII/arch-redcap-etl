/**
 * Returns WRA Baseline estimated date of delivery using Naegele’s rule.
...
CREATE FUNCTION arch_etl_db.getEstimatedDateOfDelivery_V1(p_record_id BIGINT)
    ...
    DETERMINISTIC
BEGIN
    DECLARE v_edd, v_lmp_date DATE;
    -- WRA Baseline Pregnancy Overview & Surveillance
    SELECT IF(pos_v1.lmp_start_scorres = 1, pos_v1.lmp_scdat, getEstimated_LMP_V1(p_record_id))
    INTO  v_lmp_date
    ...
    IF v_lmp_date IS NOT NULL THEN
        -- Calculate the
