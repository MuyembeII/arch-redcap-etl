 ...
 */
DROP FUNCTION IF EXISTS `getEstimatedGestationalAge_V3`;
DELIMITER $$
CREATE FUNCTION arch_etl_db.getEstimatedGestationalAge_V3(p_record_id BIGINT)
    RETURNS MEDIUMINT
    READS SQL DATA
    NOT DETERMINISTIC
BEGIN
    ...
    SELECT COALESCE(ps_v3.ps_fu_visit_date_f2, v3.visit_date),
           ps_v3.fu_lmp_scdat_f2,
           CASE ps_v3.fu_lmp_cat_scorres_f2
               WHEN 0 THEN ps_v3.fu_lmp_start_weeks_f2
               WHEN 1 THEN ps_v3.fu_lmp_start_months_f2
               WHEN 2 THEN ps_v3.fu_lmp_start_years_f2 END,
           CASE ps_v3.fu_lmp_cat_scorres_f2
               WHEN 0 THEN 'week(s)'
               WHEN 1 THEN 'month(s)'
               WHEN 2 THEN 'year(s)' END
    INTO v_current_visit_date, v_fu_lmp_date, v_estimated_fu_lmp, v_estimated_fu_lmp_flag
    FROM wrafu_pregnancy_surveillance_2 ps_v3
             JOIN arch_etl_db.crt_wra_visit_3_overview v3 on ps_v3.record_id = v3.record_id
    WHERE ps_v3.record_id = p_record_id;
    ...
    SET @v_fu_lmp_day = CAST(DAY(v_fu_lmp_date) as SIGNED); -- LMP Date Day
    SET @v_curr_day = CAST(DAY(v_current_visit_date) as SIGNED); -- Visit Date Day
    SET @v_fu_lmp_month = CAST(MONTH(v_fu_lmp_date) as SIGNED); -- LMP Date Month
