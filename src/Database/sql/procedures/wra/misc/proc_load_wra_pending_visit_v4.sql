/**
 * Load WRA FU-3 Pending Appointments.
 *
 * @author Gift Jr <muyembegift@gmail.com> | 27.02.25
 * @since 0.0.1
 * @alias WRA FU-3 Pending Appointment
 */
DROP PROCEDURE IF EXISTS `Load_WRA_Pending_Visit_V4`;

DELIMITER $$
CREATE PROCEDURE Load_WRA_Pending_Visit_V4()
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND, 1062
        BEGIN
            ...
            SET @full_error =
                    CONCAT_WS('\n', 'ERROR - Failed to load WRA FU-3 Pending Visit;', @errno, '(',
                              @sqlstate, '):',
                              @text);
    ...
    START TRANSACTION;
    TRUNCATE arch_etl_db.crt_wra_pending_visit_4_overview;

    INSERT INTO arch_etl_db.crt_wra_pending_visit_4_overview(record_id,
                                                            screening_id,
                                                            wra_ptid,
                                                            last_visit_date,
...
-- Dateset 3: 2nd FU visits due for 3rd FU appointment.
    ...
    WHERE fu3.record_id NOT IN (SELECT sc.record_id FROM crt_wra_study_closure sc)
    ORDER BY fu3.follow_up_3_visit_date_days_late DESC;
    COMMIT;

    -- flag completion
    SELECT 'WRA-FU-3-Pending-Overview-Data loader completed successfully.' as `|_________________| Operation_Summary |_________________|`;
