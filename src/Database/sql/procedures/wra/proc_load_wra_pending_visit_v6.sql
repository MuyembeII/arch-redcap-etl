/**
 * Load WRA FU-5 Pending Appointments.
 *
 * @author Gift Jr <muyembegift@gmail.com> | 27.02.25 12:37 AM
 * @since 0.0.1
 * @alias WRA FU-5 Pending Appointment
 */
DROP PROCEDURE IF EXISTS `Load_WRA_Pending_Visit_V6`;

DELIMITER $$
CREATE PROCEDURE Load_WRA_Pending_Visit_V6()
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND, 1062
        BEGIN
            ...
            SET @full_error =
                    CONCAT_WS('\n', 'ERROR - Failed to load WRA FU-5 Pending Visit;', @errno, '(',
                              @sqlstate, '):',
                              @text);
    ...
    SET @v_tx_wra_f5_pending_visit := 0;
    START TRANSACTION;
    TRUNCATE arch_etl_db.crt_wra_pending_visit_6_overview;

    INSERT INTO arch_etl_db.crt_wra_pending_visit_6_overview(record_id,
                                                             screening_id,
                                                             wra_ptid,
                                                             last_visit_date,
    ...
    WHERE fu5.record_id NOT IN (SELECT sc.record_id FROM crt_wra_study_closure sc)
    ORDER BY fu5.follow_up_5_visit_date_days_late DESC;
    COMMIT;

    SET @v_tx_wra_f5_pending_visit = (SELECT COUNT(f5.record_id) FROM arch_etl_db.crt_wra_pending_visit_6_overview f5);
    -- flag completion
    SELECT CONCAT_WS(' | ',
                     'WRA-FU-5-Pending-Overview-Data: LOADING COMPLETE',
                     CONCAT('TOTAL = [', @v_tx_wra_f5_pending_visit, ']')
