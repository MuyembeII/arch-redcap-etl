/**
 * Load WRA FU-4 Pending Appointments.
 *
 * @author Gift Jr <muyembegift@gmail.com> | 27.02.25 12:24 AM
 * @since 0.0.1
 * @alias WRA FU-4 Pending Appointment
 */
DROP PROCEDURE IF EXISTS `Load_WRA_Pending_Visit_V5`;

DELIMITER $$
CREATE PROCEDURE Load_WRA_Pending_Visit_V5()
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND, 1062
        BEGIN
            ...
            SET @full_error =
                    CONCAT_WS('\n', 'ERROR - Failed to load WRA FU-4 Pending Visit;', @errno, '(',
                              @sqlstate, '):',
                              @text);
    ...
    START TRANSACTION;
    TRUNCATE arch_etl_db.crt_wra_pending_visit_5_overview;

    INSERT INTO arch_etl_db.crt_wra_pending_visit_5_overview(record_id,
                                                            screening_id,
                                                            wra_ptid,
                                                            last_visit_date,
    ...
    WHERE fu4.record_id NOT IN (SELECT sc.record_id FROM crt_wra_study_closure sc)
    ORDER BY fu4.follow_up_4_visit_date_days_late DESC;
    COMMIT;

    -- flag completion
    SELECT 'WRA-FU-4-Pending-Visit-Overview-Data: LOADING COMPLETE' as `|_________________| Operation_Summary |_________________|`;
